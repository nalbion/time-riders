...
public class SimpleMotorbikeController : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        if ((speedVal < 0.01f && thisTransform.up.y < 0.2f && Mathf.Abs(prevOmega) < 0.001f) || (Keyboard.current.rKey != null && Keyboard.current.rKey.wasPressedThisFrame) || fallen == true) {
            reset();
        }
    ...
    private void uprightForce() {
        ...
        if (speedVal < 1 && !(Keyboard.current.wKey.isPressed || Keyboard.current.upArrowKey.isPressed)) {
    ...
    void CalcGear() {
        ...
        currentGear =  Mathf.FloorToInt(rb.linearVelocity.magnitude/13);
        if(currentGear!=prevGear)
        StartCoroutine(MotorDisengage());
        revValue = rb.linearVelocity.magnitude%13/13;
    }
